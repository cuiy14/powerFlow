...
#define f2(i,j,n) ((i-1)*(n)+j-1)		//try to understand the definition
...
void printMatrix(float * p, int rows, int cols)
{
    ...
    for(int iter=0;iter!=rows;iter++)
    {
        for(int jter=0;jter!=cols;jter++)
            std::cout<<p[iter*cols+jter]<<"\t\t";
        std::cout<<std::endl;
    }
}

void extendMatrix(float * a, int n, int m, float * jm,float * p0, float * q0, float * v0,int k)
{
...
/*	if(file4==NULL)
	...
	/* 计算接地支路导纳 */
	for(i=1;i<=n;i++)          //Pi 等效出来的接地电纳和节点直接的对地电纳；上面的部分是针对每一个支路的内容
	{
		/*　symmetrical part　*/
		b[f2(i,i,n)]+=co[f1(i)];	//the pure resistor to the ground

		/* unsymmetrical part */
		for(j=1;j<=l;j++)
		{
		  b[f2(i,i,n)]+=c[f2(i,j,l)];		// the equal circuits of the branches, not symmetry, like transformer
		}
	}
	if(k!=1)
	{
          ...
          int n,float *e,float *f,int k,float *g,float *b,float *dd)
{
	...
	*/
	l=n-1;     //redefine the parameter l
	if(k==1)
	{
